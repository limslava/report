 ...
 * (решения 15.09.2026, старт перенесён на 17.09.2026). Здесь — чистый расчёт без базы.
 *
 * - план (выгрузка/погрузка, перемещение) — заявки на дату, кроме «стоп» и «не везем»;
 *   фиксируется в 09:50 по Владивостоку этого дня (время отправки отчёта);
...
export const KTK_VVO_AUTOFILL_FROM = '2026-09-17';
...
/** Момент фиксации плана дня: 09:50 по Владивостоку этой даты. */
export const planFreezeAt = (date: string): Date => {
  const [year, month, day] = date.split('-').map(Number);
  return new Date(Date.UTC(year, month - 1, day, 9, 50) - VLAT_OFFSET_MS);
};
 ...
 * - план: пересчитывается до 09:50 дня; после — только если значения ещё нет вовсе;
