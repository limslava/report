 ...
 */
export const HH_REJECTION_REASONS = [
...
export const HH_REJECTION_REASON_CODES = HH_REJECTION_REASONS.map((reason) => reason.code);
export type HhRejectionReasonCode = typeof HH_REJECTION_REASONS[number]['code'];

export function rejectionReasonLabel(code: string | null | undefined): string | null {
  if (!code) return null;
  return HH_REJECTION_REASONS.find((reason) => reason.code === code)?.label ?? code;
}
