...
Важно:
- Убедитесь, что в настройках Amvera действительно выбрана нужная ветка.
- Если в Amvera `HEAD` указывает на `master`, а релизы вы делаете в `main`, можно получить старый деплой и `502`.
- В логе сборки строка `HEAD is now at <sha>` должна совпадать с актуальным коммитом ветки, которую вы деплоите.
...
Перед подтверждением релиза проверьте:
1. В build-логе `HEAD is now at ...` показывает ожидаемый SHA из `origin/main`.
2. В `amvera.yml`/`amvera.yaml` задан `run.containerPort: 3000`.
3. В переменных окружения приложения `APP_PORT=3000`.
...
### Почему сайт отдает `502`, хотя приложение в логах иногда стартует?
Частая причина: несоответствие ветки деплоя (например, Amvera смотрит `master`, а изменения в `main`) или старый SHA в ветке, которую реально деплоит Amvera.

Что проверить сразу:
1. В build-логе какой именно `HEAD is now at <sha>`.
2. Какой SHA у `origin/main`.
3. Что `containerPort` и `APP_PORT` совпадают (`3000`).

Быстрый recovery, если Amvera сейчас читает `master`, а актуальная версия в `main`:

```bash
git push amvera main:master
```

После этого в Amvera будет задеплоен тот же коммит, что и в `main`.
...
### Почему есть `npm error signal SIGTERM`?
Обычно это нормальное завершение старого процесса при ротации контейнера во время нового деплоя.
Считаем это проблемой только если после этого новый процесс не стартует и нет строки `Server running on port 3000`.
...
Типовой ручной спец-сценарий: Amvera по факту читает `master`, а релизный коммит лежит в `main`.
Тогда используем `git push amvera main:master`, чтобы Amvera подняла новый билд с актуального кода.
