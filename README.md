...
- Раздел `/business-processes/candidate-checks` доступен HR-рекрутеру (`hr_recruiter`), руководителю СБ и администратору.
- Роли отдела кадров `head_hr` и `hr_specialist` используются для графиков работы и не имеют доступа к БП проверки кандидатов.
- HR-рекрутер создаёт проверку кандидата и обязательно прикрепляет анкету.
...
- Дашборд `/business-processes/dashboard` разделяет задачи: СБ видит договоры и кандидатов, HR-рекрутер видит кандидатский блок с проверками на СБ и результатами.
...
- Пользовательская инструкция по БП договоров: `docs/user-guides/contract_approval_user_guide.docx` и `docs/user-guides/contract_approval_user_guide.pdf`.
