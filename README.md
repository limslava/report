...
- **Итоговый отчет (СВ‑план)** с автоматическим расчетом плана с переносом
...
- Разделение прав по направлениям
- Роли: оператор, руководитель, администратор
...
- Carry-over: `carry(m) = base(m) + max(0, carry(m-1) - fact(m-1))`.
...
- **Таблицы:** AG Grid (React Data Grid)
- **Графики:** Recharts
...
### 9. Вход в систему
Откройте `http://localhost:5173` в браузере и войдите с учётными данными:
- **Логин:** `2720233@gmail.com`
- **Пароль:** `312046`
...
- **Итоговый отчет (СВ‑план):** в меню «Отчетность» → «Сводный отчет».
...
- Возможности: управление пользователями, просмотр логов, настройка прав доступа.
...
| POST  | `/auth/register` | Регистрация (только по инвайту) |
...
| Метод | Эндпоинт | Описание |
|-------|----------|----------|
| GET   | `/data/operational/:department/:date` | Получение данных направления на дату |
| POST  | `/data/operational/:department/:date` | Сохранение/обновление данных |

### Отчеты
| Метод | Эндпоинт | Описание |
|-------|----------|----------|
| GET   | `/reports/generate/daily/:department/:date` | Генерация ежедневного отчета |
| GET   | `/reports/generate/monthly/:department/:year/:month` | Генерация месячного отчета |
| POST  | `/reports/schedule` | Настройка расписания рассылки |
| GET   | `/reports/history` | История сгенерированных отчетов |

### Администрирование
| Метод | Эндпоинт | Описание |
|-------|----------|----------|
| GET   | `/admin/users` | Список пользователей |
| POST  | `/admin/users/invite` | Приглашение нового пользователя |
| PUT   | `/admin/users/:id` | Обновление данных пользователя |
| GET   | `/admin/audit` | Просмотр логов действий |

### Метрики направлений (дополнение)
| Метод | Эндпоинт | Описание |
|-------|----------|----------|
| GET   | `/metrics/:department/:date` | Все метрики отдела на дату |
| POST  | `/metrics/:department/:date` | Обновление метрик |
| GET   | `/balances/initial` | Получение начальных остатков |
| POST  | `/balances/initial` | Установка начальных остатков |

## 🗄 Модели данных

### Основные таблицы PostgreSQL
- **users** – пользователи системы
- **operational_data** – оперативные данные по направлениям
- **monthly_plans** – месячные планы и факты
- **reports** – сгенерированные отчеты
- **email_schedules** – расписания email‑рассылок
- **department_metrics** – расчётные метрики направлений
- **initial_balances** – начальные остатки (для автовозов)
- **manual_entries** – ручной ввод финансовых показателей
